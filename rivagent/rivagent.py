...
class ScoreNode:
    def __init__(self, agent, max_depth, oap, depth, parent, agreements):
        ...
        if self.agent.use_single_neg:
            ...
            self.reserved_value = self.agent.my_ufun(self.agreements+[None]*self.rest_neg_num)

        if not self.is_leaf:
            self.next_outcome_2_child = {}
            for next_outcome in self.all_next_outcomes:
                child = ScoreNode(self.agent, self.max_depth, self.oap, 
                    depth = self.depth + 1,
                    parent = self,
    ...
    def score(self):
        ...
        if self.is_leaf:
            ...
            for next_outcome in self.descend_next_outcomes:
                ret += self.next_outcome_2_prob[str(next_outcome)] * self.get_child(next_outcome).score
...
class ScoreSpace:
    ...
    def descend_offers(self):
        return [o for o in self.root.descend_next_outcomes
            if o is not None]
...
class ThresholdAreaSpace:
    def __init__(self, agent):
        ...
        for offer in agent.score_space.descend_offers[1:]:
            ...
            self.areas[-1].rng.mn = agent.score_space.get(agent.score_space.descend_offers[-1])
...
class Threshold:
    ...
    def calc_r_mn(self, state):
        ...
        th_exp = self.agent.coeff['th_exp_concession'] \
            if self.agent.opponent_model.judge_aggressive() else self.agent.coeff['th_exp_aggressive']
...
class OpponentModel:
    ...
    def judge_aggressive(self):
        ...
        if len(self.offer_history) < window_size:
            return False
        return len(set(self.offer_history[-window_size:])) <= self.agent.coeff['opp_aggressive_th']
...
class RivAgent(ANL2025Negotiator):
    ...
    def update_neg_if_needed(self):
        ...
        if self.neg_index >= 0:
            ...
            if total_steps > self.n_steps * self.coeff['oap_rt_min1']\
